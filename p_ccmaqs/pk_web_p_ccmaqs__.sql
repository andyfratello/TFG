...
CREATE OR REPLACE PACKAGE BODY pk_web_p_ccmaqs AS
  ...
  BEGIN
    ...
    FOR c IN (SELECT
      ...
      mytable(mytable.count + 1) := pk_web_p_ccmaqs.r_of(c.id_sesion, c.codigo_org_planta, c.numero_simulacion, c.orden_de_fabricacion, c.secuencia, c.fase, c.linea, c.codigo_articulo, c.tipo, c.fecha_ini, c.fecha_fin, c.recurso, c.desc_recurso, c.porc_completado);
    END LOOP;
...
FUNCTION get_recursos_graph(p_codigo_simulacion VARCHAR2, p_codigo_org_planta VARCHAR2, p_codigo_empresa VARCHAR2, p_hasta_proyecto VARCHAR2, p_desde_proyecto VARCHAR2, p_maquina_hasta VARCHAR2,
  ...
  BEGIN
    FOR c IN (SELECT
               ...
               WHERE c.codigo_org_planta = p_codigo_org_planta
                 AND c.numero_simulacion = p_codigo_simulacion) LOOP
      mytable(mytable.count + 1) := r_recurso(c.orden_de_fabricacion, c.secuencia, c.codigo_articulo, c.recurso, c.desc_recurso);
    END LOOP;
